...
const SidebarRight = () => {
    ...
    const {curSongData, curSongId, curAlbumId} = useSelector(state => state.music) 
    useEffect(() => {
        const fecthDetailPlaylist = async () => {
            const res = await apiGetDetaiPlaylist(curSongData?.album?.encodeId)
            console.log(res);
        }
        if(curSongData?.album?.encodeId) fecthDetailPlaylist()
    }, [curSongId])
    return (
