...
const Player = () => {
  ...
  useEffect(() => {
    if(volumeRef.current) {
      volumeRef.current.style.cssText = `${volume}%`
    }
  }, [volume])
  //handle play button
      ...
      <div className="w-[30%] flex-auto text-center flex items-center justify-end">
        ...
        >
          ...
          ? <div className="w-[130px] h-1 bg-gray-200 flex-none  rounded-r-full rounded-l-full relative">
            <div ref={volumeRef} className="absolute left-0 right-0 top-0 bottom-0 bg-orange-500 rounded-r-full rounded-l-full"></div>
          </div> : 
          <input
            type="range"
            max={100}
