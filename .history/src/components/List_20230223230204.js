    ...
    >
      <div className="flex items-center gap-3 flex-1">
        ...
        <div className="flex flex-col w-full">
          <span className="text-sm font-semibold text-red-900 mb-1">
            {songData?.title?.length > 30
              ? `${songData?.title?.slice(0, 30)}...`
              : songData?.title}
          </span>
          <span className="text-red-800 text-xs">{songData?.artistsNames}</span>
        </div>
      </div>
      {!isHideNameAlbum && 
      ...
      {duration && <div className="flex-1 flex justify-end text-red-900 text-xs opacity-70">
