...
const SearchAllInfo = (searchData) => {
  ...
  return (
    <div className="w-full flex flex-col px-[60px] pb-[30px] gap-[40px]">
      ...
      <div className="flex flex-col mt-6">
        <h3 className="text-lg font-semibold text-red-600 mb-4">Album</h3>
          <div className="flex justify-between flex-wrap w-full">
            {searchData?.playlists?.map(item => (
              <div className="flex items-center justify-between gap-[28px]">
              {searchData && searchData?.playlists?.length > 0 && searchData.playlists.filter((item, index) => index <=4)?.map(item => (
                  <div 
                  key={item.encodeId}
                  onClick={() => {
