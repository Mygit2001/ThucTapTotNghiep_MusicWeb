        ...
        <div >
            {searchData?.top && 
             <div className='p-[10px] flex items-center gap-4'>
              ...
              <div className='flex flex-col'>
                <span className='text-xs text-red-500'>{searchData.top.objectType === 'artist' ? 'Nghệ sĩ' : ''}</span>
                <span className='text-sm text-red-900 font-bold'>{searchData.top.title || searchData.top.name}</span>
              </div>
             </div>
            }
        </div>
