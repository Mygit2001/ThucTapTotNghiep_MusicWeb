    ...
    <div className="w-full flex flex-col px-[60px] pb-[30px]">
      <div className="flex flex-col">
        ...
        <div className='flex gap-8'>
            {searchData?.top && 
             <div 
               className='p-[10px] flex items-center gap-6 bg-neutral-300 flex-1 rounded-md'
            ...
            {searchData?.songs?.filter((item, index) => [...Array(2).key()].some(i => i === index)).map(item => {
              <div key={item.encodeId} className='flex-1'>song1</div>
            })}
        </div>
      </div>
    </div>
